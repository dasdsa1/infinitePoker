...
class Win:
	...
	def is_pair(lst1, hand):
		...
		if len(pairs_count) > 1:
			...
			for card in hand.foo_hand:
				if len(final_hand) < 5:
					if card.face_value != pairs_count[0] or card.face_value != pairs_count[1]:
						final_hand.append(card)
					else:
						break

			return True, 8, (pairs_count[0], pairs_count[1]), final_hand
		if len(pairs_count) == 1:
			final_hand=[card for card in hand.foo_hand if card.face_value == pairs_count[0]]
			
			sorted(hand.foo_hand, key=lambda Card : Card.face_value, reverse=True)
